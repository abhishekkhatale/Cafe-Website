...
?>
<script>
    var coldCoffeeData = <?php echo json_encode($milk_coffee); ?>;
</script>
...
<head>
    ...
    <script>
        ...
        window.onload = function () { changeTitle("Milk Coffee"); };
    </script>
    <script>
        tailwind.config = {
            theme: {
    ...
    <div class="category-menu">
            ...
            <div class="category-item ">
                <a href="milk_coffee_page.php" >
                    <img src=".\stocks\menu\status2.jpg" alt="Milk Coffee" class="category-image arc">
                </a>
        ...
        <?php
for ($i = 0; $i < count($milk_coffee); $i++) {
    $imageName = './stocks/menu/MENU_img/milk_coffee_img/' . $milk_coffee[$i]['imgurl'] . '.jpg';
    $price = $milk_coffee[$i]['price'];
    
    echo '
    <div class="card">
        <div class="card-image">
            <img src="' . $imageName . '" alt="' . $milk_coffee[$i]['name'] . '" class="product-image" onclick="openModal(' . $i . ')">
            <h2 class="coffee-break">Coffee Break</h2>
            <span class="discount">SAVE UP TO ' . $milk_coffee[$i]['discount'] . '%</span>
        </div>
        <div class="card-content">
            <div class="product-name-veg-icon">
                <h3 class="product-name" onclick="openModal(' . $i . ')">' . $milk_coffee[$i]['name'] . '</h3>
                <span class="veg-icon"><img src="./menu_visual/veg.png" alt=""></span>
            </div>
            <p class="offer">' . $milk_coffee[$i]['desc'] . '</p>
            <input type="hidden" name="product_price" value="' . $price . '">
             <p class="price">₹ ' . $price  . '</p>
            <div class="rating">';

    // Loop for displaying stars
    $stars = $milk_coffee[$i]['star'];
    for ($j = 1; $j <= 5; $j++) {
